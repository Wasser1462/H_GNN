...
class GNNModel(nn.Module):
    def __init__(self, input_dim, hidden_dim_0, hidden_dim_1, output_dim, num_heads=1):
        ...
        self.W0 = nn.Parameter(torch.Tensor(input_dim, hidden_dim_0 * num_heads))
        self.b0 = nn.Parameter(torch.Tensor(hidden_dim_0 * num_heads))
        
        self.W1 = nn.Parameter(torch.Tensor(hidden_dim_0 * num_heads, hidden_dim_1 * num_heads))
        self.b1 = nn.Parameter(torch.Tensor(hidden_dim_1 * num_heads))
    ...
    def forward(self, data):
        ...
        x = F.relu(x)
        x = self.attention_mechanism(x, edge_index)
